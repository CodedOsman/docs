...
| **Description** | In this project, learners will use a 74HC595 shift register to control eight LEDs in a chasing sequence using only three Arduino pins. This project introduces the concept of serial-to-parallel data output and pin expansion, which is essential when a project requires more output devices than the Arduino has available pins.
Learners will discover how data is shifted out bit by bit to control all eight LED outputs, and how this technique scales in real embedded systems designs.|
|------------------|----------------------------------------------------------------|
...
- Place the 74HC595 Shift Register and the 8 LEDs with 220Ω resistors on the breadboard.
...
- Check that all GND connections share the same ground line before powering the circuit.

- Check the polarity of all LEDs and ensure each LED has a 220Ω current-limiting resistor before connecting USB power.
